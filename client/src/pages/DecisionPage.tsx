...
export default function DecisionPage() {
  ...
  if (loading && !app) return <DataState.Loading label="Loading decision…" />;
  if (error || !app) return <DataState.Error message={error ?? 'Unavailable'} onRetry={refresh} />;
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-lg font-extrabold tracking-tight text-navy-900">Decision & Next Steps</h1>
          <p className="text-[12px] text-charcoal-500">The recommendation, the reasoning, and the single action that unblocks the case</p>
        </div>
