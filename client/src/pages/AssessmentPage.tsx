...
export default function AssessmentPage() {
  ...
  if (loading && !app) return <DataState.Loading label="Loading explainable assessment…" />;
  if (error || !app) return <DataState.Error message={error ?? 'Unavailable'} onRetry={refresh} />;
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-lg font-extrabold tracking-tight text-navy-900">Explainable Decision</h1>
          <p className="text-[12px] text-charcoal-500">Why ClearGov thinks what it thinks for {app.applicationId}</p>
        </div>
