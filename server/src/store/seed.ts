...
const SCENARIO_EVIDENCE: Record<string, Array<{ req: string; seed: string }>> = {
  scholarship: [
...
function scenarioForKey(key: string): Scenario {
  return SCENARIOS.find((s) => s.key === key)!;
}

function evidenceFor(app: ApplicationRecord, req: string, seed: string, fileName: string, days: number): Evidence {
...
export function createSeed(): StoreSnapshot {
  ...
  const counters: Record<string, number> = { applicationSeq: 161 };

  const now = new Date().toISOString();

  // ------------------------------------------------------------------ flagship
  const flagshipApp: ApplicationRecord = {
    id: uuid(),
  ...
  const fsAudit = (action: string, reason: string, previous?: string, details?: Record<string, unknown>, days = 4) => {
    audit.push({ id: uuid(), applicationId: flagshipApp.id, actor: 'System', role: 'AI', action, timestamp: daysAgo(days, 9), reason, previousState: previous, newState: details?.newState as string, details });
  };
  ...
  notifications.push(
    { id: uuid(), userId: 'u-reviewer', applicationId: flagshipApp.id, applicationRef: 'CG-2026-0148', type: 'CONFLICT', title: 'New conflict detected', message: 'Address conflict between income certificate and residence proof for CG-2026-0148.', read: false, createdAt: daysAgo(4) },
    { id: uuid(), userId: 'u-citizen', applicationId: flagshipApp.id, applicationRef: 'CG-2026-0148', type: 'MISSING_EVIDENCE', title: 'Missing evidence requested', message: 'Bank proof is missing. Upload it to end the hold.', read: false, createdAt: daysAgo(3) },
    { id: uuid(), userId: 'u-citizen', applicationId: flagshipApp.id, applicationRef: 'CG-2026-0148', type: 'REVIEWER_REQUEST', title: 'Residence proof needs correction', message: 'The residence address conflicts with your income certificate. Reviewer R. Priya requested a corrected copy.', read: false, createdAt: daysAgo(3) },
    { id: uuid(), userId: 'u-citizen', applicationId: flagshipApp.id, applicationRef: 'CG-2026-0148', type: 'ASSESSMENT', title: 'Assessment generated', message: 'ClearGov established 3 of 5 conditions. 2 actions are required to proceed.', read: false, createdAt: daysAgo(4) },
  );

  // ------------------------------------------------------------------ support apps
  const seedApps: Array<{
    ref: string; scenario: string; name: string; days: number; submitted: boolean;
    overrides?: Array<{ req: string; seed: string }>;
    humanReview?: boolean; resolved?: boolean; highIncome?: boolean;
    extraRoute?: boolean; lowQualityResidence?: boolean; missing?: string[];
  }> = [
  ...
  for (const s of seedApps) {
    ...
    if (s.submitted) {
      for (const e of base) {
        if (s.missing?.includes(e.req)) continue;
        let seed = e.seed;
        if (e.seed === 'income' && s.highIncome) seed = 'income_high';
        if (e.seed === 'residence' && s.lowQualityResidence) seed = 'residence_lowquality';
        app.evidence.push(evidenceFor(app, e.req, seed, `${s.name}_${seed}.pdf`, Math.max(1, s.days - 1)));
      }
      if (s.extraRoute) app.evidence.push(evidenceFor(app, 'route', 'route_b', `${s.name}_tertiary_declaration.pdf`, Math.max(1, s.days - 1)));
  ...
  notifications.push(
    { id: uuid(), userId: 'u-reviewer', applicationId: apps.find((a) => a.applicationId === 'CG-2026-0151')!.id, applicationRef: 'CG-2026-0151', type: 'HUMAN_REVIEW', title: 'Human review required', message: 'Land ownership is ambiguous. Reviewer judgment required for CG-2026-0151.', read: false, createdAt: daysAgo(4) },
    { id: uuid(), userId: 'u-citizen', applicationId: apps.find((a) => a.applicationId === 'CG-2026-0152')!.id, applicationRef: 'CG-2026-0152', type: 'MISSING_EVIDENCE', title: 'Evidence processed', message: 'Income verified for CG-2026-0152. Bank proof is still required.', read: false, createdAt: daysAgo(3) },
    { id: uuid(), userId: 'u-reviewer', applicationId: apps.find((a) => a.applicationId === 'CG-2026-0155')!.id, applicationRef: 'CG-2026-0155', type: 'CONFLICT', title: 'New conflict detected', message: 'Commute route declarations conflict for CG-2026-0155.', read: false, createdAt: daysAgo(4) },
    { id: uuid(), userId: 'u-citizen', applicationId: apps.find((a) => a.applicationId === 'CG-2026-0153')!.id, applicationRef: 'CG-2026-0153', type: 'RESOLVED', title: 'Issue resolved', message: 'All enrollment records verified for CG-2026-0153.', read: true, createdAt: daysAgo(2) },
  );
