import type { Scenario, ScenarioDef, RequirementDef } from './types.js';
...
const scholarshipRequirements: RequirementDef[] = [
  {
    key: 'identity',
    label: 'Identity',
...
function genericRequirements(keys: Array<{ key: string; label: string; evidence: string; field: string; min: number }>): RequirementDef[] {
  return keys.map((k) => ({
    key: k.key,
    label: k.label,
