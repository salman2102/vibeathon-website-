...
function latestEvidence(app: ApplicationRecord, requirementKey: string) {
  return app.evidence
    .filter((e) => e.requirementKey === requirementKey && e.status === 'PROCESSED' && !e.superseded)
    .sort((a, b) => (a.uploadedAt < b.uploadedAt ? 1 : -1));
}

function reqDef(scenarioKey: string, key: string) {
  const sc = SCENARIOS.find((s) => s.key === scenarioKey);
  return sc?.requirements.find((r) => r.key === key);
}

function ruleText(req: ReturnType<typeof reqDef>): string {
  if (!req) return '';
  const r = req.rule;
  if (r.type === 'threshold') {
    let th = String(r.threshold);
    if (r.field === 'annual_income') th = `₹${Number(r.threshold).toLocaleString('en-IN')}`;
    if (r.field === 'marks_percent') th = `${r.threshold}%`;
...
function detectCrossDocumentConflicts(app: ApplicationRecord) {
  const byKey: Record<string, Array<{ value: string; label: string; file: string; evidenceId: string; confidence: number }>> = {};
  ...
  for (const ev of current) {
    for (const f of ev.extractedFields) {
      if (['id_number', 'account_number', 'ifsc'].includes(f.key)) continue;
      (byKey[f.key] ??= []).push({ value: f.value, label: f.label, file: ev.fileName, evidenceId: ev.id, confidence: f.confidence });
    }
  }
  const raw: Array<{ key: string; label: string; pairs: Array<{ value: string; file: string; evidenceId: string; confidence: number }> }> = [];
  ...
  const clusters: Array<{ keys: string[]; label: string; pairs: Array<{ value: string; file: string; evidenceId: string; confidence: number }> }> = [];
  for (const c of raw) {
...
function assessmentConfidence(status: RequirementStatus, evidenceConf: number, issueConf: number): number {
  switch (status) {
    case 'ESTABLISHED':
...
export function analyzeApplication(app: ApplicationRecord): ApplicationRecord {
  ...
  const requirements: RequirementStatusRecord[] = [];
  ...
  for (const req of SCENARIOS.find((s) => s.key === app.scenarioKey)?.requirements ?? []) {
    ...
    const evidenceConf = evidence
      ...
      : 0;
    const ruleFacts: Record<string, ExtractedFact> = {};
    for (const f of facts) if (f.requirementKey === req.key) ruleFacts[f.key] = f;
    if (evidence) for (const f of evidence.extractedFields) ruleFacts[f.key] = { ...f, requirementKey: req.key, category: 'evidence', source: evidence.fileName } as ExtractedFact;
    ...
    const assessment: Assessment = {
      ...
      explanation: {
        ...
        nextAction:
          status === 'MISSING_EVIDENCE' ? `Upload a valid ${req.evidenceLabel.toLowerCase()}.` :
    ...
    requirements.push({
      ...
      evidenceIds: [evidence?.id].filter(Boolean) as string[],
  ...
  const coverageFor = (st: RequirementStatus) =>
    st === 'ESTABLISHED' ? 100 : st === 'CONFLICT' ? 90 : st === 'LOW_CONFIDENCE' ? 55 : st === 'NOT_ESTABLISHED' ? 25 : st === 'HUMAN_REVIEW' ? 65 : 0;
  ...
  const status: AppStatus = app.manualStatus === 'RESOLVED'
    ? 'RESOLVED'
    : humanReviewNeeded
  ...
  const SUMMARY: Record<AppStatus, string> = {
...
function buildNextActions(app: ApplicationRecord, reqs: RequirementStatusRecord[], status: ApplicationRecord['status']): NextAction[] {
...
function missingLabel(scenarioKey: string, reqKey: string): string {
  const sc = SCENARIOS.find((s) => s.key === scenarioKey);
  const req = sc?.requirements.find((r) => r.key === reqKey);
  return req?.evidenceLabel ?? 'Evidence';
}

export function decideStatus(conflicts: number, missing: number, lowConf: number, notEst: number, humanReview: boolean): ApplicationRecord['status'] {
  if (humanReview) return 'HUMAN_REVIEW_REQUIRED';
  if (missing > 0 || conflicts > 0 || lowConf > 0) return 'ADDITIONAL_EVIDENCE_REQUIRED';
  if (notEst > 0) return 'CONDITION_NOT_ESTABLISHED';
