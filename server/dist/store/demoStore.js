...
export class DemoStore {
    ...
    audit(appId, actor, role, action, reason, previousState, newState, details) {
        const ev = {
            id: uuid(),
            applicationId: appId,
            actor,
    ...
    notify(userId, applicationId, applicationRef, type, title, message) {
        const n = { id: uuid(), userId, applicationId, applicationRef, type, title, message, read: false, createdAt: new Date().toISOString() };
    ...
    listApplications(filters) {
        ...
        if (filters.search) {
            ...
            rows = rows.filter((a) => a.applicationId.toLowerCase().includes(q) ||
        ...
        if (filters.scenario)
            rows = rows.filter((a) => a.scenarioKey === filters.scenario);
        if (filters.issueType)
            rows = rows.filter((a) => a.issues.some((i) => i.type === filters.issueType));
        if (filters.reviewer)
            rows = rows.filter((a) => (a.assignedReviewer ?? a.reviewedBy) === filters.reviewer);
        if (confidenceMin)
            rows = rows.filter((a) => a.aiConfidence >= confidenceMin);
        if (dateFrom)
            rows = rows.filter((a) => a.createdAt >= dateFrom);
        if (dateTo)
            rows = rows.filter((a) => a.createdAt <= dateTo);
    ...
    getApplicationByRef(ref) {
        const app = this.getApplication(ref);
        if (!app)
            return { app: undefined, ok: false, error: 'Application not found' };
        return { app, ok: true };
    }
    // ================================================================ evidence
    addEvidence(ref, input) {
        const app = this.getApplication(ref);
        if (!app)
            throw new Error('Application not found');
        const req = SCENARIOS.find((s) => s.key === app.scenarioKey)?.requirements.find((r) => r.key === input.requirement);
        if (!req)
            throw new Error(`Unknown requirement "${input.requirement}" for scenario ${app.scenarioKey}`);
    ...
    reviewerAction(ref, input) {
        ...
        if (input.action === 'MARK_ISSUE_RESOLVED' && input.issueId) {
            ...
            if (issue?.requirementKey)
                app.suppressedIssueKeys.push(issue.requirementKey);
            analyzeApplication(app);
    ...
    markAllRead(userId) {
        this.snapshot.notifications.forEach((n) => {
            if (n.userId === userId || n.userId === 'u-citizen')
                n.read = true;
        });
        this.persist();
    }
    ...
    analytics() {
        ...
        const statusDist = [];
        const order = ['DRAFT', 'SUBMITTED', 'ADDITIONAL_EVIDENCE_REQUIRED', 'HUMAN_REVIEW_REQUIRED', 'CONDITION_NOT_ESTABLISHED', 'READY_TO_PROCEED', 'RESOLVED'];
        for (const s of order)
            statusDist.push({ name: s, value: apps.filter((a) => a.status === s).length });
        const issueTypes = new Map();
        apps.forEach((a) => a.issues.filter((i) => i.status === 'OPEN').forEach((i) => issueTypes.set(i.type, (issueTypes.get(i.type) ?? 0) + 1)));
        const conflictCategories = [...issueTypes.entries()].map(([name, value]) => ({ name, value }));
        const healthDist = [
